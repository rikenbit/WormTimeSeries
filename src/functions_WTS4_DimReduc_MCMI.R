...
.dist_f = function(x) {
    ...
    # get filter list
    dist_matrix %>% 
        attr(., "Labels") -> celllist
    celllist[grep("^[0-9]", celllist, invert=TRUE)] -> celllist
    # filter by usedistパッケージ
    return_object <- dist_subset(dist_matrix, celllist)
    return(return_object)
}
...
.plot_dimreduc = function(x) {
    ...
    df_merged <- DF_cord_cls_nl_be[[sample_n]]
    #### ggplot Cluster####

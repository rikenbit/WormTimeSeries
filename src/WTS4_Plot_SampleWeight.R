...
gg <- gg + geom_boxplot(outlier.shape = NA) #外れ値のプロットを省く
gg <- gg + geom_point(position = position_jitter(width=0.05))
gg <- gg + labs(colour="each sample") + xlab("each sample")
gg <- gg + 
    ggtitle("weight beeswarm (sort by weight_average)") +

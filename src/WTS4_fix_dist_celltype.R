...
# args_dist_sample <- c("output/WTS4/normalize_1/stimAfter/SBD_abs/Distance/SampleNumber_26.RData")
# args_output <- c("output/WTS4/normalize_1/stimAfter/SBD_abs/Distance_fix/SampleNumber_14.RData")
...
# if文で判定 HYPL9VR4
d_celltype <- attr(d, "Labels")
if_HYPL9VR4 <- length(which(!is.na(match(d_celltype,"HYPL9VR4"))))
if(if_HYPL9VR4==1){
    attr(d, "Labels")[which(!is.na(match(d_celltype,"HYPL9VR4")))] <- c("HYPL9VR")
}

#### save####
save(d, file=args_output)
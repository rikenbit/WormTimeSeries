...
eval(parse(text=paste0("inputpath <- paste(path,
                       inputdir,
                       'ReadData_",args_sample,".RData',
                       sep = '/')")
           )
     )
load(inputpath)
# ReadData[,'108'] %>% as.numeric() -> nactivity
eval(parse(text=paste0("ReadData[,'",args_celltype,"'] %>% as.numeric() -> nactivity")))
#TimeFrame
# rownames(ReadData) %>% as.numeric() -> timeframe
eval(parse(text=paste0("rownames(ReadData) %>% as.numeric() -> timeframe")))
...
path <- "data"
inputdir <- "mCherry"
# inputpath <- paste(path, inputdir, 'mCherry_3.RData', sep = '/')
...
path <- "data"
inputdir <- "Position"
# inputpath <- paste(path, inputdir, 'Position_3.RData', sep = '/')

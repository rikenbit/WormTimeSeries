...
rule DistMatrix:
    ...
    output:
        dist_matrix = 'output/WTS3/normalize_1/{range}/{dist}/SampleNumber_{N}/{dist}.RData'
    params:
        stim_xlsx = 'data/stimulation/stimulation_timing.xlsx'
    benchmark:
        'benchmarks/WTS3/normalize_1/{range}/{dist}/SampleNumber_{N}/{dist}.txt'
    conda:
        '../envs/myenv_WTS3_{dist}.yaml'
    resources:
        mem_gb=200
    log:
        'logs/WTS3/normalize_1/{range}/{dist}/SampleNumber_{N}/{dist}.log'
    shell:
        'src/WTS3_{wildcards.dist}.sh {wildcards.N} {input.RData} {wildcards.range} {params.stim_xlsx} {output.dist_matrix}>& {log}'